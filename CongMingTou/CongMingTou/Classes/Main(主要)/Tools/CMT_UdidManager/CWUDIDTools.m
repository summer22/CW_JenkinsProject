...
//  IZhuJia
//
//  Created by 陈森的Apple on 15-5-27.
...
@implementation CWUDIDTools

+ (NSString*)UDID
{
    NSString *udid = [CWUDIDTools getUDIDFromKeyChain];
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [CWUDIDTools _UDID_iOS7];
        }
        else if (version >= 2.0) {
            udid = [CWUDIDTools _UDID_iOS6];
        }
        
        [CWUDIDTools settUDIDToKeyChain:udid];
    }
...
+ (NSString*)_UDID_iOS6
{
    return [CWUDIDTools getMacAddress];
}
...
//        DDLog(@"KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainUDIDItemIdentifier]);
    }
    else if (queryErr != errSecSuccess) {
//        DDLog(@"KeyChain Item query Error!!! Error code:%d", queryErr);
    }
    if (queryErr == errSecSuccess) {
//        DDLog(@"KeyChain Item: %@", udidValue);
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([CWUDIDTools getUDIDFromKeyChain]) {        // there is item in keychain
        [CWUDIDTools updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
...
//            DDLog(@"Add KeyChain Item Error!!! Error Code:%d", (int)writeErr);
            
            [dictForAdd release];
            return NO;
        }
        else {
//            DDLog(@"Add KeyChain Item Success!!!");
            [dictForAdd release];
            return YES;
        }
    }
...
//        DDLog(@"delete UUID from KeyChain Error!!! Error code:%ld", deleteErr);
        [dictToDelete release];
        return NO;
    }
    else {
//        DDLog(@"delete success!!!");
    }
...
//            DDLog(@"Update KeyChain Item Error!!! Error Code:%ld", updateErr);
...
//            DDLog(@"Update KeyChain Item Success!!!");
            [dictForQuery release];
            [dictForUpdate release];
            return YES;
        }
