...
#define SHEIGHT 2 // 中间指示器顶部闭合三角形高度
...
- (void)setInputValueAutoMoveScroll{
    CGFloat offSetX = rulerScrollView.contentOffset.x + self.frame.size.width / 2 - DISTANCELEFTANDRIGHT;
    CGFloat ruleValue = (offSetX / DISTANCEVALUE) * [rulerScrollView.rulerAverage floatValue];
    if (ruleValue < 0.f) {
        ruleValue = 0;
    } else if (ruleValue > rulerScrollView.rulerCount * [rulerScrollView.rulerAverage floatValue]) {
        ruleValue = rulerScrollView.rulerCount * [rulerScrollView.rulerAverage floatValue];
    }
    if (self.rulerDeletate) {
        if (!rulerScrollView.mode) {
            rulerScrollView.rulerValue = ruleValue;
        }
        rulerScrollView.mode = NO;
        [self.rulerDeletate txhRrettyRuler:rulerScrollView];
    }
}
...
- (void)drawRacAndLine {
    ...
    CGPathAddQuadCurveToPoint(pathArc, NULL, self.frame.size.width / 4, - 20, self.frame.size.width, DISTANCETOPANDBOTTOM);
    ...
    CGPathMoveToPoint(pathLine, NULL, self.frame.size.width / 2, self.frame.size.height - DISTANCETOPANDBOTTOM);
