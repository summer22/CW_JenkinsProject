...
- (void)initialize
{
	...
	self.highlightedTextColor = self.highlightedTextColor ?: ThemeBlueColor;
...
- (void)selectItem:(NSUInteger)item animated:(BOOL)animated notifySelection:(BOOL)notifySelection
{
	...
	if (notifySelection &&
		[self.delegate respondsToSelector:@selector(pickerView:didSelectItem:size:)])
		[self.delegate pickerView:self didSelectItem:item size:self.spaceSize];
}
