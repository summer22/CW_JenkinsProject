...
#import "ZHCleanCaches.h"

@implementation ZHCleanCaches
...
+ (BOOL)clearCachesFromDirectoryPath:(NSString *)dirPath{
    //获得全部文件数组
    NSArray *fileAry =  [ZHCleanCaches getAllFileNames:dirPath];
    //遍历数组
    BOOL flag = NO;
    for (NSString *fileName in fileAry) {
        NSString *filePath = [dirPath stringByAppendingPathComponent:fileName];
        flag = [ZHCleanCaches clearCachesWithFilePath:filePath];
        if (!flag)
            break;
    }
    return flag;
